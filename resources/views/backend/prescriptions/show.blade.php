...
@push('css')
    <style>
        ...
        @media print {
            ...
            .prescription-wrapper { border: none !important; box-shadow: none !important; }
        ...
        .data-value { border-bottom: 1px dashed #ccc; display: inline-block; min-width: 50px; padding: 0 5px; }
        
        .medicine-table th { background-color: #f8f9fa !important; font-size: 12px; }
...
@section('content')
    <div class="container py-4">
        ...
        <div class="d-flex justify-content-between mb-3 d-print-none">
            ...
            <a href="{{ route('prescriptions.print', $prescription->id) }}" class="btn btn-secondary btn-sm px-3">
            print</a>
        ...
        <div class="prescription-wrapper shadow-sm">
            ...
            <div class="row bg-white py-2 mb-3 border border-dark mx-0">
                <div class="col-4">
                    <span class="data-label">Name:</span> <span class="data-value fw-bold">{{ $prescription->patient_name }}</span>
                </div>
                <div class="col-2">
                    <span class="data-label">Age:</span> <span class="data-value">{{ $prescription->patient_age }}</span>
                </div>
                <div class="col-2">
                    <span class="data-label">Wt:</span> <span class="data-value">{{ $prescription->patient_weight ?? 'N/A' }}</span>
                </div>
                <div class="col-2">
                    <span class="data-label">Gender:</span> <span class="data-value text-capitalize">{{ $prescription->patient_gender }}</span>
                </div>
                <div class="col-2 text-end">
                    <span class="data-label">Date:</span> <span class="data-value">{{ $prescription->prescription_date->format('d-m-Y') }}</span>
                </div>
            </div>
            ...
            <div class="row border border-dark mx-0" style="min-height: 800px;">
                ...
                <div class="col-3 border-end border-dark p-3" style="font-size: 12px; background-color: #fafafa;">
                    ...
                    <div class="mb-4">
                        ...
                        {{-- Bowel --}}
                        @if(isset($symp['bowel']))
                            <div class="symptom-tag"><i class="fa-solid fa-caret-right"></i> Bowel: {{ implode(', ', $symp['bowel']) }}</div>
                        @endif
                        ...
                        @foreach(['general', 'urine', 'others'] as $key)
                            @if(isset($symp[$key]))
                                @foreach($symp[$key] as $item)
                                    <div class="symptom-tag"><i class="fa-solid fa-caret-right"></i> {{ $item }}</div>
                                @endforeach
                            @endif
                        @endforeach
                        ...
                        @foreach($prescription->items as $index => $item)
                            <div class="mb-3">
                                <div class="d-flex gap-2 align-items-baseline">
                                    ...
                                    <div>
                                        <span class="fw-bold text-uppercase" style="font-size: 15px;">{{ $item->product_name }}</span>
                                        <br>
                                        <span class="text-muted small">({{ $item->generic_name }})</span>
                                    </div>
                                </div>
                                <div class="ms-4 mt-1">
                                    <span class="fw-bold">{{ $item->dosage_data }}</span> 
                                    --- {{ $item->dosage_unit }} 
                                    --- {{ $item->dosage_time == 'after-meal' ? 'খাবার পরে' : 'খাবার আগে' }}
                                    @if($item->duration)
                                        --- {{ $item->duration }} {{ $item->duration_type }}
                                    @endif
                                </div>
                                @if($item->instructions)
                                    <div class="ms-4 mt-1 text-danger small italic">
                                        * {{ $item->instructions }}
                                    </div>
                                @endif
                            </div>
